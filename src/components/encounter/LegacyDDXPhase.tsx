...
export function LegacyDDXPhase({ ddxResults, factCount, isComputing, onAccept, onComplete, onManualAdjust }: LegacyDDXPhaseProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {ddxResults.map((d, i) => {
          ...
          const isTop = i === 0;
          return (
            <div key={d.diseaseId} className={`p-3 border rounded-lg transition-colors ${isTop ? 'border-blue-300 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
              ...
              <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${isTop ? 'bg-blue-500' : 'bg-gray-300'}`} style={{ width: `${barWidth}%` }} />
              </div>
              {d.keyFactors.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {d.keyFactors.map((f, j) => (
                    <span key={j} className="px-2 py-0.5 text-xs bg-gray-100 text-gray-600 rounded-full">{f}</span>
                  ))}
                </div>
              )}
            </div>
